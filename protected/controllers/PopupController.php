...
class PopupController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update', 'viewjs'),
				'users'=>array('@'),
			),
	...
	public function actionView($id)
	{
		...
		$count_views = $model->attributes['views'];

		if($status == '1') $model->updateByPk($id, array("views" => $count_views + 1)	);
...
//		$this->render('view', array('model'=>$model));
	 ...
	 * @param integer $id the ID of the model to be displayed
	 */
	public function actionViewjs($id)
	{
		if( !is_numeric($id) ) return false;
...
/*
		...
		echo json_encode($data, true);
*/
		$this->render('viewjs', array('model'=>$model));
	}
	...
	public function actionCreate() {
		...
		if(isset($_POST['Popup'])){
			...
			if($model->save())
				...
				$this->redirect(array('viewjs','id'=>$model->id));
		}
